...
| South Carolina (SC) | ✅ | ✅ | Utilized `ethicsfiling.sc.gov` to gather contribution, expenditure, and financial report (candidates/public officials only). Additional party, office, and jurisdiction information was extracted from `electionhistory.scvotes.gov`. Standalone PACs are a separate, opt-in source (`--pacs`, `apps.sc.gov`) — see [docs/states/south_carolina.md](docs/states/south_carolina.md#non-candidate-committees-pacs) |
