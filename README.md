...
| North Dakota (ND) | ⚠️ | ✅ | CFRS (`cfrs.sos.nd.gov`), pure HTTP, no Playwright. **Two acquisition paths:** the Data Download catalog (JSON listing → signed CloudFront link → streaming GET) for Contributions, Expenditure, Registration (filer roster, feeding both committees and candidates), FiledReports (joined on `(RegistrantID, FiledDate)` for `amended`/`filing_id`) and ReportingSchedule (archived, unread); plus the public "Get to Know" DataGrid export, which is the **only** source of office/district/party/address and is joined on by name since it carries no filer ID (479/482 registrations enriched — office 98.6%, district 91.0%, party 82.8%). Parser verified against real 2025/2026 downloads and all three endpoints' payloads/envelopes confirmed from captured traffic (the grid returns the workbook inline as base64 in `responseData.fileBytes`); **the scraper has not yet made a live round trip**, though its HTTP logic has 78 offline assertions. No treasurer/jurisdiction/incumbency and no loan export in source. **Coverage is 2025-onward only** — CFRS launched Jan 2026; 2024-and-prior sits in a separate legacy ASP.NET archive (`cf.sos.nd.gov`) with no bulk export, not scraped. ND is migrating history in through 2027, so no start year is hardcoded — the catalog defines what exists. **Independent expenditures are a known gap:** absent from the bulk exports entirely, and the public IE grid is 76% duplicate contributions and multiply-counts the remainder, so it is documented rather than wired up. Addresses arrive as one concatenated field in two different formats and are split by the parser. **Uses `verify=False`** — CFRS serves a broken certificate chain (sends its leaf twice, omits the intermediate CA), so OpenSSL cannot validate it on any machine; same pattern as `alabama.py`. The server's certificate fingerprint is checked each run and a change is warned about, not enforced. See [docs/states/north_dakota.md](docs/states/north_dakota.md) |
