...
overlay — see the "Party/office enrichment overlay" section below, fed by
`scrapers/new_york.py --party`, which joins NYSBOE's own election-results
...
# Join NYSBOE election results and Open States onto the New York candidate
# registry to fill party, incumbent, district and election_year.
#
# Inputs (both optional, written by this same file's --party mode — see the
# CLI block at the bottom):
#     data/New York/raw/ElectionStats_Contests.csv
#     data/New York/raw/OpenStates_People.csv
#
# If neither exists this section is inert: every lookup returns "no answer"
# and the parser writes exactly the blanks it writes without it. Enrichment
# must never be load-bearing for a state's parse to succeed.
#
# Matching contract
# ------------------
# Strict only. A match requires the candidate's **name and canonical office**
# to agree, plus **either the district or the election year**. There is no
# nickname expansion, no soundex, no edit distance, and no single-token
# surname matching — NY has 36,486 candidate filers over three decades and
# enough repeated names ("Eric A. Ulrich" alone holds seven filer_ids) that
# fuzzy matching would attach wrong parties to real people at a rate nobody
# downstream could detect.
#
# Every enriched field is written alongside two provenance columns so a
# consumer can set their own bar rather than inheriting ours:
#
...
# Fusion voting
# -------------
# New York lets several parties nominate the same candidate, each getting its
# own ballot line. `party` is therefore multi-valued: every line that
# candidacy carried, canonicalised through src/aliases/parties.csv and joined
# with "|", ordered by votes received on that line (descending) so the
# candidate's primary line reads first and the ordering is deterministic:
#
...
# contests. Town, village, city and school-district races are certified by
# the 62 county boards and are absent from the results database. Those local
# offices are most of the NY filer registry, so party fill on `candidates` is
# capped structurally — not by this matcher. `coverage_report()` returns the
# split so the number is visible in the parse log rather than being mistaken
...
def _enrich_party_list(raw_lines) -> str:
...
class Candidacy:
    ...
    @property
    def party(self) -> str:
        return _enrich_party_list(self.lines)
        ...
        with open(path, newline="", encoding="utf-8", errors="replace") as f:
            for row in csv.DictReader(f):
                ...
                cand.lines.append(((row.get("party") or "").strip(),
                                   _enrich_int(row.get("votes"))))
                if _enrich_truthy(row.get("is_winner")):
                    cand.won = True
    ...
    try:
        ...
        if enrich.available:
            ...
            log.warning("  No enrichment overlay found in "
                        ...
                        "Run scrapers/new_york.py --party to populate them.")
