...
**person_id model:** `committee` — the FCPA system assigns a new `CommitteeId` per committee registration. `person_id` is set to the earliest `CommitteeId` seen for a given candidate name + office + district, prefixed with Alabama's FIPS code (01), producing 13-digit integers.
