...
`src/pipeline/scrapers/texas_party.py`

A separate, optional scraper from `texas.py` above — different hosts, different cadence, never load-bearing for the main TEC pull. It downloads the two fallback sources `parsers/texas_enrich.py` joins onto `candidates.party` wherever TEC's own `cover.csv` leaves it blank: the Texas SOS's legacy race-summary canvass (`elections.sos.state.tx.us`, 1992–2019) and Open States' nightly bulk CSV (`data.openstates.org/people/current/tx.csv`, current TX Legislature — CC0, no API key needed, same unauthenticated export the New York enrichment already uses). `--sos` / `--openstates` scope which of the two run; no flag runs both. Writes `SOS_RaceSummary.csv` and `OpenStates_People.csv` to `data/Texas/raw/` alongside TEC's own files — neither overwrites or is read by `scrapers/texas.py` itself. See the module docstring for exactly how each source's URL/page structure was confirmed, and `docs/states/texas.md`'s Data Notes section above for the matching contract.
...
- **`party` comes from `cover.csv` first, with a two-source fallback behind it.** `filers.csv` has no party column, but the report cover sheets do: TEC's record layout (CFS-ReadMe.txt, Record #4 `CoverSheet1Data`) declares `politicalPartyCd` (DEM, REP, LIB…), `politicalPartyOtherDescr`, `politicalDivisionCd` and `politicalPartyCountyCd`, all keyed on `filerIdent` — an exact join, no name matching. The parser reads it and takes each filer's most recently declared party.
  ...
  Whatever that join leaves blank falls through to `scrapers/texas_party.py` + `parsers/texas_enrich.py`, an overlay tried in this priority order (first strict match wins — see the module docstrings for the full matching contract):
